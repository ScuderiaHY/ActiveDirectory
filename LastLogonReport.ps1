...
{   Write-Verbose "$(Get-Date): Retrieving users from domain controller $DC in $Domain"
    ...
    $Jobs += Start-Job -ArgumentList $SearchBase,$DC -ScriptBlock {
        ...
        Try {
            Get-ADUser @Params | Where{ $_.Name -notmatch "(^HealthMailbox|^SystemMailbox)"} | Select Surname,GivenName,Name,distinguishedName,LastLogon,SamAccountName,@{Name="DC";Expression={$DC}},Enabled,PwdLastSet
        }
        Catch {
            [PSCustomObject]@{
                DC = $DC
...
If ($Age)
{   $Results = $Users.Values | Where LastLogonDate -LE ((Get-Date).AddDays($Age)) | Sort LastLogonDate -Descending
}
Else
